...
export default function HighlightToolbar({ position, onHighlight, onLookup, onClose }: HighlightToolbarProps) {
  const adjustedPosition = {
    x: Math.min(Math.max(position.x, 140), window.innerWidth - 140),
    y: Math.max(position.y - 10, 40),
  }
  ...
  return (
    <motion.div
      ...
      exit={{ opacity: 0, y: 10 }}
      style={{ position: 'fixed', left: adjustedPosition.x, top: adjustedPosition.y, transform: 'translate(-50%, -100%)', zIndex: 3000 }}
    >
      <div style={{
        ...
        {onLookup && (
          <>
            ...
            <button
              ...
              style={{
                width: 28, height: 28, borderRadius: 6,
                border: 'none', background: `${C.accent}20`,
