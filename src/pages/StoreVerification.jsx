...
export default function StoreVerification({ navigate }) {
  const [phone, setPhone] = useState('');
  const [selectedCountry, setSelectedCountry] = useState(
    COUNTRY_CODES.find(c => c.country === 'India') || COUNTRY_CODES[0]
  );
  ...
  const confirmOtp = async () => {
    ...
    try {
      ...
      setVerified(true);
      await setDoc(doc(db, 'storeVerifications', auth.currentUser.uid), {
  ...
  const goBack = () => navigate('business');

  return (
    <div className="store-verification">
      <button className="icon-btn" onClick={goBack} id="store-verif-back-btn">
        <ArrowLeft size={18} />
      </button>
      <h2>Verify Phone Number</h2>
      <p className="verify-subtitle">We&apos;ll send a 6-digit OTP to verify your identity</p>
      ...
      {verified ? (
        <div className="verified-success">
          <div className="success-icon-ring">
            <Check size={48} color="#00e676" />
          </div>
          <p>Phone verified! You can now open a store.</p>
          <button className="glow-btn" onClick={goBack} id="store-verif-done-btn">
        ...
        <>
              ...
              <div className="country-list">
                {filteredCountries.map((c, i) => (
                  <button
                    ...
                    className={`country-option ${c.country === selectedCountry.country ? 'active' : ''}`}
          ...
          <button className="glow-btn" onClick={startVerification} disabled={loading} id="send-otp-btn">
          ...
          {confirmationResult && (
            <>
              <input
                ...
                id="store-otp-input"
              />
              <button className="glow-btn" onClick={confirmOtp} disabled={loading} id="verify-otp-btn">
