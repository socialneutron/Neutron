...
const C = {
  bg: '#05050A', text: '#f1f5f9', muted: '#6b7280',
...
export default function BusinessPage({ navigate, user, initialTab }) {
  ...
  return (
    <div style={{ height: '100vh', display: 'flex', flexDirection: 'column', background: C.bg }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 18px', borderBottom: '1px solid rgba(255,255,255,0.06)', background: '#05050A' }}>
        <span style={{ fontSize: 16, fontWeight: 900, color: C.text, letterSpacing: '2px' }}>neutron</span>
        <span style={{ fontSize: 13, color: C.muted }}>Business</span>
        <div style={{ flex: 1 }} />
      ...
      <div style={{ flex: 1, minHeight: 0 }}>
        <MarketplaceApp navigate={navigate} user={user} initialTab={initialTab} />
      </div>
    </div>
  )
}
